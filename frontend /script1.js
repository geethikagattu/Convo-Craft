/**
 * ConvoCraft – Immersive hero + slideshow (and optional app logic when present).
 */
...
(function () {
  ...
  function initSlideshow() {
    var container = document.getElementById('hero-bg-slides');
    if (!container || !HERO_IMAGES.length) return;

    // Create one img per URL, layered (position: absolute in CSS)
    HERO_IMAGES.forEach(function (url, i) {
      var img = document.createElement('img');
      img.className = 'hero-bg-img' + (i === 0 ? ' active' : '');
      img.src = url;
      img.alt = '';
    ...
    // Preload then start interval (no page jump; images already in DOM)
    preloadImages(HERO_IMAGES, function () {
  ...
  function onGenerateIcebreaker() {
    ...
    const contextKey = situationKeyMap[contextValue] || 'College';
    const personalityKey = personalityKeyMap[personalityValue] || 'Introvert';

    const situationData = conversationData[contextKey];
    const lines = (situationData && situationData[personalityKey]) || [];

    showIcebreakerLoading(true);
    setTimeout(() => {
      const chosen = getRandomUniqueItems(lines, 3);
      if (chosen.length === 0) {
        showIcebreakerResults(['No suggestions for this combination yet. Try another situation or style.']);
      } else {
        showIcebreakerResults(chosen);
      }
    }, 1000);
  }
  ...
  function onSuggestReplies() {
    ...
    setTimeout(function () {
      const natural = pickRandom(NATURAL_FOLLOWUPS, 3);
      const empathy = pickRandom(EMPATHY_RESPONSES, 1)[0];
      const humor = pickRandom(HUMOR_RESPONSES, 1)[0];
  ...
  function sendChatMessage() {
    ...
    setTimeout(function () {
      appendBubble(getAIResponse(text), false);
    }, 400);
